...
//       GoblinPortalTerminalView → TerminalPane (via processDelegate, but that is not
//       a responder) → window → app
//     `GoblinPortalTerminalView` inherits `MacTerminalView`, which inherits `TerminalView`,
...
//
//  3. KEY EQUIVALENT. ⌘F reaches the menu item because `GoblinPortalTerminalView.
